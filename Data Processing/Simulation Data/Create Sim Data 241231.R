...
simulate_vanco <- function(n_subj, age_mean, age_sd, wt_mean, wt_sd, cr_mean, cr_sd, 
  ...
  AGE <- rnorm(n_subj, age_mean, age_sd)
  WT <- rnorm(n_subj, wt_mean, wt_sd)
  Cr <- rnorm(n_subj, cr_mean, cr_sd)
  ...
  # Output
  list(ground_truth = ground_truth, observation = observation)
}
...
write.csv(ground_truth, "ground_truth_241231.csv", row.names = FALSE)
write.csv(observation, "observation_241231.csv", row.names = FALSE)
